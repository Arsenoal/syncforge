...
| 1.0-P0-03 | **Publish `1.0.0` to Maven Central** — core artifacts: `syncforge`, `annotations`, `ksp`, `persistence`, `android-deps`, Gradle plugin                                                                                                                     | Dist ✅    |
...
**Version catalog (1.4+):** `:syncforge-catalog` is the recommended consumer pin — one `libs.versions.toml` import aligns the Gradle plugin and all optional library artifacts. Most apps declare only `syncforge.core` (KSP is pinned by `studio.syncforge.android`; runtime modules are transitive). See [GETTING_STARTED.md](GETTING_STARTED.md).

**Optional artifacts:** Store, network, transport, and integration modules are published separately and listed in the catalog — not transitive in `:syncforge`. Add explicit dependencies when needed.
...
| 1.1-11 | **Store adapter modules** — optional artifacts, not transitive in `:syncforge`         | P1       | e.g. `:syncforge-store-room` (current DAO path), `:syncforge-store-realm`, in-memory for tests |
...
| [#B3](.) | `feat(store): :syncforge-store-room adapter module` | 1.1-11 | B2 | Room DAO → `EntityStore` bridge published; optional artifact, not transitive |
...
| 1.4-05 | **Gradle version catalog for consumers**              | P1       | Published `:syncforge-catalog`; primary consumer version alignment (see [Distribution notes](#distribution-notes-10--20)) ✅ |
...
| ID     | Job                                                                 | Priority | Notes |
|--------|---------------------------------------------------------------------|----------|-------|
| 1.6-00 | **Web platform spike** — Wasm vs JS, persistence + Ktor engine PoC  | P0       | Go/no-go doc; monorepo-only (no Maven publish) ✅ |
...
- **Maven Central publish** for web `js` artifacts — integrate via git clone, composite build, or `publishToMavenLocal` only
...
| 4 | Version catalog lists all library aliases + plugin | `:syncforge-catalog:verifyCatalogArtifacts` | ✅ in `verifyReleaseSignOff` |
...
Prerequisites: Sonatype namespace, signing secrets — [MAVEN_PUBLISH.md](MAVEN_PUBLISH.md). **Do not** use `-PallowPre2MavenCentralPublish=true` for the release tag.
...
| # | Criterion | Verification | Status |
|---|-----------|--------------|--------|
| 27 | SPM publish pipeline implemented | `publishIosSpmArtifacts` uploads (not stub); [IOS_SETUP.md](IOS_SETUP.md) SPM consumer path | ⬜ (stub today — may defer to `2.0.1`) |
| 28 | XCFramework or SPM package resolves for Swift consumers | Documented install + sample `ios-sample/` smoke | ⬜ |

#### Post-publish verification

After Central sync (~15–60 minutes):
...
Or **Actions → Verify Maven Central Release** — version `2.0.0`.

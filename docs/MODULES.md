...
Ready-made [GraphQlSyncTransport](../syncforge-transport-graphql/README.md) — GraphQL-over-HTTP `syncPush` mutation and `syncPull` query (same semantics as REST). Point `GraphQlSyncConfig.endpointUrl` at your schema or `:mock-server` `/graphql` for local dev.

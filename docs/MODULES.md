...
BaaS / hosted-backend adapter (1.4). Implement `SyncDeltaStore` once per vendor (Firestore, Supabase, custom RPC); wire with a single `SyncTransport`. Full guide: [CUSTOM_TRANSPORT.md](CUSTOM_TRANSPORT.md).
...
Ready-made [GraphQlSyncTransport](../syncforge-transport-graphql/README.md) — GraphQL-over-HTTP `syncPush` mutation and `syncPull` query (same semantics as REST). Schema and resolver recipes: [syncforge-server/graphql/README.md](../syncforge-server/graphql/README.md).
...
GraphQL-only Ktor reference backend on port `8080`. Exposes `POST /graphql` via
`graphqlRoutes` — pair with `:syncforge-transport-graphql`. Schema:
[syncforge-server/graphql/syncforge-sync.graphql](../syncforge-server/graphql/syncforge-sync.graphql).
See [backend-starter-graphql/README.md](../backend-starter-graphql/README.md).

...
4. **SQLDelight is the internal persistence backend on all platforms** since 0.6.0. Legacy Room schema remains only for `RoomToSqlDelightMigrator` (removed `useRoomPersistence()` in 1.0).
...
- [x] Legacy Room internals marked `internal`; `useRoomPersistence()` removed (1.0)

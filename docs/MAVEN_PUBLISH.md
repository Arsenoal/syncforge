...
| `finalizeMavenCentralStaging` **Read timed out** | Sonatype staging API can be slow after large KMP uploads. **Actions → Finalize Maven Central Staging** (retry without re-publish), or locally: `MAVEN_CENTRAL_USERNAME=… MAVEN_CENTRAL_PASSWORD=… ./gradlew finalizeMavenCentralStaging`. Defaults: 600s read timeout, 5 retries (override via `MAVEN_CENTRAL_STAGING_*` env vars). Ensure the release tag includes the staging retry fix (retag `v2.0.0` if the workflow checked out an older commit). |
